...
async function processWorkData() {
    try {
        ...
        if (workList) workList.innerHTML = '<tr><td colspan="8" class="error">Kunde inte h\xe4mta data!</td></tr>';
    }
}
function displayWorkData(data) {
    ...
    workList.innerHTML = '';
    if (data.length === 0) {
        workList.innerHTML = '<tr><td colspan="8" style="text-align: center;">Inga arbetserfarenheter finns. L\xe4gg till en!</td></tr>';
        return;
    }
    data.forEach((work)=>{
...
async function deleteWork(id) {
    if (!confirm("\xc4r du s\xe4ker p\xe5 att du vill ta bort denna arbetserfarenhet?")) return;
    try {
